...
- Gestures reserved to the Owner, never executed by an agent even when authorized: `push`, permanent deletion (agent substitute: move to `_trash/` at the root of the workspace, [Decision 110852](./decisions/DECISION-2026-08-29-110852-deletion-is-owner-gesture-trash-zone.md)). Require a human gate for any structuring rename, sensitive sharing or other sensitive action. The push may be delegated to an Executor window by a verbatim and dated Owner authorization ([Decision 154553](./decisions/DECISION-2026-08-26-154553-delegated-push-exception-becomes-rule.md)); without that exact line, the Executor refuses.
