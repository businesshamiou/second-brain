...
- `VIDEO_TIER` — default is `seedance_2_0` **via Monid** (pay-per-clip; previz =
  ...
  anything renders.
- `STILLS_SOURCE` — the user-approved image-generation capability or provider. Use one
  source for all stills in a build; mixing generators can create visible style drift.
