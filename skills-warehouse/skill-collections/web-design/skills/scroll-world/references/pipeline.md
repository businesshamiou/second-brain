# Pipeline: provider adapters and copy-paste scripts (bash 3.2 safe)

The commands below document tested provider adapters from the archived source. They are examples, not runtime requirements or current capability guarantees. Before using one, verify the provider's current model schema, pricing, balance, authentication state, and data-handling terms; obtain approval for cost and external transfer. A different provider is valid when it satisfies the same pinned-frame and continuity invariants.
...
When the runtime exposes a native image-generation capability, pass it the same prompt
files and request 3:2 PNG output at `$WORK/still_<name>.png`. Keep one generator for the
whole set and limit concurrency to what that capability supports. This capability path
replaces, rather than wraps, the provider-specific command above.
